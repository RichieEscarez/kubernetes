...
function source_file_name() {
	local version=$1
	if [ "${version}" == "api" ]; then
		echo "pkg/api/types.go"
	else
		echo "pkg/api/${version}/types.go"
	fi
}
...
function result_file_name() {
	local version=$1
	if [ "${version}" == "api" ]; then
		echo "pkg/api/types_swagger_doc.go"
	else
		echo "pkg/api/${version}/types_swagger_doc.go"
	fi
}

function generate_version() {
	local version=$1
	local TMPFILE="/tmp/types_swagger_doc.$(date +%s).go"
...
EOF
	...
	GOPATH=$(godep path):$GOPATH go run cmd/genswaggertypedocs/swagger_type_docs.go -s `source_file_name ${version}` -f - >>  $TMPFILE

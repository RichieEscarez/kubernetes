...
func writeFuncHeader(b *buffer, structName string, indent int) {
	format := "func (%s) SwaggerDoc() map[string]string {\n"
	s := fmt.Sprintf(format, structName)
	b.addLine(s, indent)
	b.addLine("return map[string]string{\n", indent+1) // Map definition
}

func writeFuncFooter(b *buffer, indent int) {
	b.addLine("}\n", indent+1) // Closes the map definition
	b.addLine("}\n", indent)
}
